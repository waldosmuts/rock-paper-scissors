...
export default function Header({ score }) {
    ...
    function RulesGraphic() {
        return (
            <svg className="h-full md:h-auto mt-[90px] md:mt-0 md:pb-8" xmlns="http://www.w3.org/2000/svg" width="304" height="270">
                <defs>
                    <linearGradient id="a" x1="50%" x2="50%" y1="0%" y2="100%">
                        <stop offset="0%" stopColor="#FFF" stopOpacity=".097" />
                        <stop offset="100%" stopColor="#FFF" stopOpacity="0" />
                    </linearGradient><linearGradient id="b" x1="50%" x2="50%" y1="0%" y2="100%">
                        <stop offset="0%" stopColor="#FFF" stopOpacity=".097" />
                        <stop offset="100%" stopColor="#FFF" stopOpacity="0" />
    ...
    return (
        <header className="font-barlow font-semibold text-white pt-7 px-7 md:pt-12 md:px-12 z-10">
            <div className={`absolute left-0 top-0 bg-black ${showRules ? "opacity-50" : "opacity-0 pointer-events-none"} transition duration-300 w-full h-full z-20`} />
            <div className={`absolute top-0 md:top-1/2 md:-translate-y-1/2 left-0 md:left-1/2 md:-translate-x-1/2 ${showRules ? "opacity-100" : "opacity-0 pointer-events-none"} transition duration-300 flex flex-col items-center md:justify-end h-screen md:h-[420px] w-full md:w-[400px] pt-24 pb-16 md:p-0 bg-white md:rounded-xl z-30`}>
                <h2 className="uppercase text-dark text-3xl font-bold md:absolute md:top-8 md:left-8">Rules</h2>
                <RulesGraphic />
                <button className="md:absolute md:top-8 md:right-8" onClick={toggleRules}><img src={CloseIcon} alt="Hide Rules" /></button>
            </div>
            <div className="flex justify-between max-w-[700px] md:mx-auto border-[3px] border-header rounded-lg md:rounded-2xl p-[11px] md:p-[15px] h-24 md:h-36">
                <a className="cursor-pointer p-2" href="."><img className="h-full" src={Logo} alt="Home" /></a>
                <div className="bg-white px-6 md:px-12 rounded-[4px] md:rounded-lg flex flex-col items-center justify-center">
                    <h2 className="text-score uppercase text-[9px] md:text-base font-bold tracking-[0.125rem] leading-none">Score</h2>
                    <span className="text-dark text-[40px] md:text-6xl font-bold md:-mt-1 leading-none">{showScore}</span>
                </div>
            </div>
            <button onClick={toggleRules} className={`${showRules && "pointer-events-none"} fixed bottom-14 md:bottom-8 left-1/2 md:left-auto md:right-8 -translate-x-1/2 md:translate-x-0 tracking-[0.2rem] uppercase font-semibold border md:border-2 border-white rounded-md md:rounded-lg px-8 py-2 hover:border-transparent transition duration-300 hover:bg-white hover:text-dark`}>Rules</button>
        </header>
    )
}